...
template <unsigned int DIM> PetscErrorCode MatExpre<DIM>::_setup() {
  ...
  if constexpr (DIM == 2) {
    // Because now we control the DMDA, we do not need to use DMDAGetCorners.
        ...
        for (unsigned int i = 0; i < 2; ++i) {
          if (xy[i] < 0.0) {
            ...
            PetscAssert(gsl_status != GSL_EDOM, PETSC_COMM_WORLD,
                        ...
                        MAGIC_CONSTANT2_W);
            aWxyz_2d[i][ey][ex] = std::sqrt(1.0 / gsl_pow_4(cn) - 1.0);
          }
          ...
          for (unsigned int i = 0; i < 3; ++i) {
            if (xyz[i] < 0.0) {
              ...
              PetscAssert(gsl_status != GSL_EDOM, PETSC_COMM_WORLD,
                          "Fail to performing cn(%.5e, %.5e)!\n", u,
                          MAGIC_CONSTANT2_W);
              aWxyz_3d[i][ez][ey][ex] = std::sqrt(1.0 / gsl_pow_4(cn) - 1.0);
            }
  ...
  eta = DEFAULT_ETA;
  tau = DEFAULT_TAU;
  num_quad_points = DEFAULT_QUAD_POINTS;
...
template <unsigned int DIM> PetscErrorCode MatExpre<DIM>::get_mat(Mat A) {
  ...
  if constexpr (DIM == 2) {
    ...
    for (PetscInt ey = local_0_start; ey < local_0_start + local_n0; ++ey) {
      for (PetscInt ex = 0; ex < total_elems[0]; ++ex) {
        PetscReal v = avelocity_2d[ey][ex].real(), w = aW_2d[ey][ex].real();
        MatStencil row = {0, ey, ex, 0};
        MatStencil cols[MAX_STENCIL_2D] = {
            {0, ey, ex, 0},
            {0, ey, ex - 1 == -1 ? total_elems[0] - 1 : ex - 1, 0},
            {0, ey, ex + 1 == total_elems[0] ? 0 : ex + 1, 0},
            {0, ey - 1 == -1 ? total_elems[1] - 1 : ey - 1, ex, 0},
            {0, ey + 1 == total_elems[1] ? 0 : ey + 1, ex, 0}};
        // Laplacian operator.
  ...
  if constexpr (DIM == 3) {
    ...
    for (PetscInt ez = local_0_start; ez < local_0_start + local_n0; ++ez) {
      for (PetscInt ey = 0; ey < total_elems[1]; ++ey) {
        for (PetscInt ex = 0; ex < total_elems[0]; ++ex) {
          PetscReal v = avelocity_3d[ez][ey][ex].real(),
                    w = aW_3d[ez][ey][ex].real();
          MatStencil row = {ez, ey, ex, 0};
          MatStencil cols[MAX_STENCIL_3D] = {
              {ez, ey, ex, 0},
              {ez, ey, ex - 1 == -1 ? total_elems[0] - 1 : ex - 1, 0},
              {ez, ey, ex + 1 == total_elems[0] ? 0 : ex + 1, 0},
              {ez, ey - 1 == -1 ? total_elems[1] - 1 : ey - 1, ex, 0},
              {ez, ey + 1 == total_elems[1] ? 0 : ey + 1, ex, 0},
              {ez - 1 == -1 ? total_elems[2] - 1 : ez - 1, ey, ex, 0},
              {ez + 1 == total_elems[2] ? 0 : ez + 1, ey, ex, 0}};
          // Laplacian operator.
...
PetscErrorCode MatExpre<DIM>::_apply_exp_diag(PetscScalar coeff) {
  ...
  if constexpr (DIM == 3) {
    ...
    for (auto ez = local_0_start; ez < local_0_start + local_n0; ++ez)
      for (auto ey = 0; ey < total_elems[1]; ++ey)
        for (auto ex = 0; ex < total_elems[0]; ++ex) {
          ...
          auto offset = (ez - local_0_start) * total_elems[1] * total_elems[0] +
                        ey * total_elems[0] + ex;
          PetscScalar *data_ptr =
              reinterpret_cast<PetscScalar *>(&fftw_data[offset]);
          *data_ptr *= std::exp(coeff * d);
...
template <unsigned int DIM>
PetscErrorCode MatExpre<DIM>::_apply_exp_A(PetscScalar s) {
  PetscFunctionBeginUser;

  // exp(s/2 D)
  PetscCall(_apply_exp_diag(s * 0.5));

  // exp(s L)
  fftw_execute(forward_plan);
  _apply_exp_laplace_freq(s);
...
template <unsigned int DIM>
PetscErrorCode MatExpre<DIM>::pc_apply(Vec input, Vec output) {
  const PetscReal *quad_pos = nullptr, *quad_wgh = nullptr;
  switch (num_quad_points) {
  case 3:
    quad_pos = GL_QUAD_POS_3;
    quad_wgh = GL_QUAD_WGH_3;
    break;
  case 4:
    quad_pos = GL_QUAD_POS_4;
    quad_wgh = GL_QUAD_WGH_4;
    break;
  case 5:
    quad_pos = GL_QUAD_POS_5;
    quad_wgh = GL_QUAD_WGH_5;
    break;
  default:
    PetscCall(PetscPrintf(PETSC_COMM_WORLD,
                          "Use the default num_quad_points=%d\n.",
                          DEFAULT_QUAD_POINTS));
    num_quad_points = DEFAULT_QUAD_POINTS;
    quad_pos = GL_QUAD_POS_3;
    quad_wgh = GL_QUAD_WGH_3;
  ...
  PetscCall(_DMDA_vec_to_FFTW_vec(input));

  // The first quandrature point.
  PetscCall(VecCopy(input, output));
  PetscCall(VecScale(output, quad_wgh[0]));
  for (auto i = 1; i < num_quad_points; ++i) {
    // next quandrature point.
    PetscReal delta_t = quad_pos[i] - quad_pos[i - 1];
    PetscCall(_apply_exp_A(IU * delta_t * 0.5 * tau));
    PetscCall(VecAXPY(fftw_vec_forward_input, quad_wgh[i], output));
  }
  // The final scaling in int_{-1}^{1} ... dt
  PetscCall(VecScale(output, tau * 0.5));
  // Now, the output is updated to int_0^tau exp(i s A) (input) ds.

  // Fixed-point iteration part, essentially, we need to solve
  // (I - exp(i tau A))^{-1} output.

  // output <- output + exp(i tau A) output
  PetscCall(_DMDA_vec_to_FFTW_vec(output));
  PetscCall(_apply_exp_A(IU * tau));
  PetscCall(VecAXPY(fftw_vec_forward_input, 1.0, output));
...
PetscErrorCode pc_apply_2d(PC pc, Vec input, Vec output) {
  MatExpre<2> *matexpre = nullptr;
  PetscFunctionBeginUser;
...
PetscErrorCode pc_apply_3d(PC pc, Vec input, Vec output) {
  MatExpre<3> *matexpre = nullptr;
  PetscFunctionBeginUser;
  PetscCall(PCShellGetContext(pc, reinterpret_cast<void **>(&matexpre)));
  PetscCall(matexpre->pc_apply(input, output));
  PetscFunctionReturn(PETSC_SUCCESS);
